...
do
  for kernel in 'rbf' 'matern1-2'  'matern5-2';
  do
    python error_vs_sample_size.py  --kernel $kernel --estimate $method --load --csv_file "out/error_vs_sample_size/$kernel--$method.csv" --save_fig "img/error_vs_sample_size/$kernel--$method.png"  --style styles/vr.mpl
  done
done
